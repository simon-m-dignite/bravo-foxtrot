  ...
  return (
    ...
    >
      <div className="w-full horizontal-padding flex items-center justify-end gap-x-6 absolute top-6">
        <Link
          to="/contact-us"
          className="px-5 py-2 outline-none rounded-full text-sm font-medium flex items-center bg-yellow-600 text-white"
        >
          Contact Us
          <TiArrowRight className="text-2xl" />
        </Link>
        <button onClick={onclick} className="">
          <IoClose className="text-3xl text-yellow-600" />
        </button>
      </div>

      <ul className="flex flex-col items-start gap-6 2xl:gap-5 text-black px-12 justify-center h-full">
        <li
          className="text-2xl lg:text-5xl quantico-fonts font-extrabold secondary-text cursor-pointer hover:text-yellow-600 transition-all duration-300"
      ...
      <div className="w-full absolute bottom-6 flex items-center justify-end gap-x-5 horizontal-padding">
        ...
        >
          <TiSocialFacebook className="text-yellow-600 text-lg" />
        </Link>
        <Link target="_blank" to={"https://www.instagram.com"}>
          <FaInstagram className="text-yellow-600 text-base" />
        </Link>
        <Link target="_blank" to={"https://www.twitter.com"}>
          <RiTwitterXLine className="text-yellow-600 text-base ml-1" />
        </Link>
      </div>
    </div>
  );
